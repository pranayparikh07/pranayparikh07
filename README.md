---
<p align="center">
  <img src="https://img.shields.io/badge/PHP-777BB4?style=for-the-badge&logo=php&logoColor=white"/>
  <img src="https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black"/>
  <img src="https://img.shields.io/badge/Java-ED8B00?style=for-the-badge&logo=java&logoColor=white"/>
  <img src="https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white"/>
  <img src="https://img.shields.io/badge/Firebase-FFCA28?style=for-the-badge&logo=firebase&logoColor=black"/>
  <img src="https://img.shields.io/badge/Prompting-10A37F?style=for-the-badge&logo=openai&logoColor=white"/>
</p>

<p align="center">
  <em>Diploma Student in Information Technology | Aspiring Software Engineer | Backend Developer | Cybersecurity Enthusiast</em>
</p>

---

## 🧠 About Me

Hello! I'm Pranay Parikh, a passionate and curious second-year Diploma IT student dedicated to crafting smart, scalable, and secure software systems. From developing intuitive CRUD applications to designing fantasy-themed dashboards, I thrive on blending creativity with efficient, clean code. My journey is fueled by a relentless drive to innovate and solve real-world problems through technology.

> *"I don't just write code. I architect digital experiences."*

### 🚀 What Drives Me

- 🔐 Exploring the depths of **cybersecurity** and **ethical hacking** to build resilient systems.
- ⚙️ Building robust applications with **Java**, **PHP**, **Node.js**, and **MongoDB**.
- 🎨 Crafting user interfaces inspired by **cricket teams** and **fantasy realms** for immersive experiences.
- 📊 Architecting **serverless apps**, **AI-driven systems**, and **themed dashboards** that push boundaries.
- 🧠 Constantly learning, testing, debugging, and iterating to deliver excellence.

### 🌍 Portfolio

<p align="center">
  <a href="https://pranayparikh.vercel.app" target="_blank">
    <img src="https://img.shields.io/badge/Portfolio-Visit-blueviolet?style=for-the-badge"/>
  </a>
</p>

---

## 💼 Work Experience

### **Web Developer** | *RemoteDevs Infotech*
- Developed and enhanced an advanced task management system using PHP, boosting operational efficiency by 30%.
- Engineered user-centric features that improved system usability, scalability, and overall user satisfaction.

### **Quality Assurance Engineer** | *Lucres & Deepak Technocast*
- Conducted comprehensive testing and quality assurance to ensure software reliability, performance, and compliance with industry standards.

---

## 🎨 Featured Projects

### 📱 **[SecondWave Phones](https://github.com/pranayparikh07/SecondWave-Phones)**
*An intuitive online platform for streamlined acquisition of pre-owned smartphones.*
- **Sustainable Marketplace**: Promotes affordability and environmental sustainability by encouraging electronics reuse.
- **Trust & Quality**: Implements rigorous quality checks to guarantee high functionality and trustworthy standards.
- **User-Centric Design**: Features a seamless interface for smooth transactions, transparent pricing, and an exceptional user experience.
- **Tech Stack**: PHP, MySQL, HTML/CSS, JavaScript.

<p align="center">
  <img src="https://media.giphy.com/media/SWoSkN6DxTszqIKEqv/giphy.gif" alt="Coding Animation" width="400"/>
</p>

### 🤖 **[HireGenius](https://github.com/pranayparikh07/hiregenius)**
*An AI-driven platform to streamline and automate recruitment and interview processes.*
- **Automated Recruitment**: Automates scheduling and enforces strict time management for maximum efficiency.
- **AI Insights**: Integrates AI-powered resume parsing and session analysis for actionable post-interview insights.
- **Cyberpunk UI**: Showcases a futuristic, engaging interface that enhances user interaction.
- **Data-Driven Decisions**: Empowers HR teams with analytics to optimize hiring processes.
- **Tech Stack**: Node.js, MongoDB, JavaScript, AI APIs.

<p align="center">
  <img src="https://media.giphy.com/media/l0HlA0zHM9AMeb4VW/giphy.gif" alt="AI Animation" width="400"/>
</p>

<details>
  <summary>See more projects...</summary>
  <ul>
    <li><a href="https://github.com/pranayparikh07?tab=repositories" target="_blank">GitHub Repositories</a></li>
  </ul>
</details>

---

## 🎓 Education

### **Diploma in Information Technology**
*Gyanmanjari Innovative University*
- **SGPA**: 8.333
- **Roles**: Team Lead and Principal Engineer at Tech Manjari 2K24 & 2K25.
- **Activities**: Coordinated industry visits to ISRO, AMUL, and Qtonz, fostering practical learning and networking.

### **Matriculation**
*Shree Swaminarayan English School*
- Achieved **81.81 percentile rank** in SSC board examinations.

---

## 🏆 Awards & Recognition

- **TechManjari 2K25**: Recognized for technical excellence, leadership, and innovative contributions.
- **TechManjari 2K24**: Awarded for outstanding participation in technical events and pioneering solutions.

---

## 💼 Tech Stack

<details>
  <summary><b>Languages</b></summary>
  <p>
    <img src="https://img.shields.io/badge/Java-ED8B00?style=flat-square&logo=java&logoColor=white"/>
    <img src="https://img.shields.io/badge/PHP-777BB4?style=flat-square&logo=php&logoColor=white"/>
    <img src="https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black"/>
    <img src="https://img.shields.io/badge/HTML5-E34F26?style=flat-square&logo=html5&logoColor=white"/>
    <img src="https://img.shields.io/badge/CSS3-1572B6?style=flat-square&logo=css3&logoColor=white"/>
    <img src="https://img.shields.io/badge/SQL-4479A1?style=flat-square&logo=postgresql&logoColor=white"/>
    <img src="https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white"/>
  </p>
</details>

<details>
  <summary><b>Tools & Frameworks</b></summary>
  <p>
    <img src="https://img.shields.io/badge/Node.js-339933?style=flat-square&logo=nodedotjs&logoColor=white"/>
    <img src="https://img.shields.io/badge/JDBC-007396?style=flat-square&logo=java&logoColor=white"/>
    <img src="https://img.shields.io/badge/MySQL-4479A1?style=flat-square&logo=mysql&logoColor=white"/>
    <img src="https://img.shields.io/badge/MongoDB-47A248?style=flat-square&logo=mongodb&logoColor=white"/>
    <img src="https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white"/>
    <img src="https://img.shields.io/badge/Vercel-000000?style=flat-square&logo=vercel&logoColor=white"/>
  </p>
</details>

<details>
  <summary><b>Productivity & Design</b></summary>
  <p>
    <img src="https://img.shields.io/badge/ClickUp-7B68EE?style=flat-square&logo=clickup&logoColor=white"/>
    <img src="https://img.shields.io/badge/Slack-4A154B?style=flat-square&logo=slack&logoColor=white"/>
    <img src="https://img.shields.io/badge/Canva-00C4CC?style=flat-square&logo=canva&logoColor=white"/>
    <img src="https://img.shields.io/badge/ChatGPT-10A37F?style=flat-square&logo=openai&logoColor=white"/>
  </p>
</details>

---

## 📊 GitHub Stats & Activity

<div align="center">
  <a href="https://github.com/pranayparikh07">
    <img src="https://github-readme-stats.vercel.app/api?username=pranayparikh07&show_icons=true&theme=github_dark&hide_title=true&count_private=true&include_all_commits=true" height="180" alt="Stats" />
  </a>
  <a href="https://github.com/pranayparikh07">
    <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=pranayparikh07&layout=compact&theme=github_dark&hide_title=true&langs_count=6" height="180" alt="Top Languages" />
  </a>
</div>
<div align="center">
  <a href="https://github.com/pranayparikh07">
    <img src="https://streak-stats.demolab.com?user=pranayparikh07&theme=github-dark&hide_border=true&date_format=M%20j%5B%2C%20Y%5D" alt="GitHub Streak" />
  </a>
</div>

---

## 📫 Connect with Me

<p align="center">
  <a href="https://pranayparikh.vercel.app" target="_blank"><img src="https://img.shields.io/badge/Portfolio-Visit-blueviolet?style=flat-square"/></a>
  <a href="mailto:pranayparikh07@gmail.com"><img src="https://img.shields.io/badge/Email-Contact-red?style=flat-square&logo=gmail&logoColor=white"/></a>
  <a href="https://linkedin.com/in/pranayparikh07" target="_blank"><img src="https://img.shields.io/badge/LinkedIn-Connect-blue?style=flat-square&logo=linkedin&logoColor=white"/></a>
  <a href="https://github.com/pranayparikh07" target="_blank"><img src="https://img.shields.io/badge/GitHub-Follow-black?style=flat-square&logo=github&logoColor=white"/></a>
</p>

---

<p align="center">
  <em>🧠 “Either write something worth reading or code something worth committing.”<br>
  🎯 Let's build something extraordinary. One commit at a time.</em>
</p>

---
